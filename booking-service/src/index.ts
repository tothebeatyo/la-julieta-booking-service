...
console.log("La Julieta Booking Service starting...");
console.log("Chromium path: /usr/bin/chromium");
...
app.get("/health", (_req, res) => {
  res.json({ ok: true, service: "La Julieta Booking", time: new Date().toISOString() });
});

app.post("/book", auth, async (req, res) => {
  const { name, service, date, time, phone, clientType } = req.body;
  console.log("Booking request:", { name, service, date, time, clientType });
  ...
  try {
    ...
    console.log("Navigating to AnyPlusPro...");
    ...
    if (loginFailed) throw new Error("Login failed — check credentials");
    console.log("Login successful!");

    await page.evaluate(() => {
      const el = Array.from(document.querySelectorAll("a,button,div,li"))
        .find((e: any) => e.textContent?.toLowerCase().includes("appointment"));
    ...
    await page.evaluate(() => {
      const btn = Array.from(document.querySelectorAll("button"))
        .find((b: any) => b.textContent?.toLowerCase().includes("appointment"));
      if (btn) btn.click();
    });
    await delay(3000);
    ...
    if (isOld) {
      ...
      if (si) {
        ...
        await page.evaluate((n: string) => {
          ...
          const m = r.find((x: any) => x.textContent?.toLowerCase().includes(n.toLowerCase()));
          if (m) (m as HTMLElement).click();
          else if (r[0]) (r[0] as HTMLElement).click();
        }, name.split(" ")[0]);
      }
    } else {
      await page.evaluate(() => {
        const el = Array.from(document.querySelectorAll("button,a,span,div"))
          .find((e: any) => e.textContent?.trim().toLowerCase().includes("register new"));
        if (el) (el as HTMLElement).click();
      });
      await delay(2000);
      ...
      if (phone) {
        await page.evaluate((p: string) => {
          const i = Array.from(document.querySelectorAll("input"))
            .find((x: any) => x.placeholder?.includes("9XX") || x.type === "tel");
          if (i) {
            (i as HTMLInputElement).value = p;
      ...
      await page.evaluate(() => {
        const sel = Array.from(document.querySelectorAll("select"))
          .find((s: any) => s.closest("div")?.textContent?.toLowerCase().includes("lead"));
        if (sel) {
          const opt = Array.from((sel as HTMLSelectElement).options)
            .find((o: any) => /facebook|social|online/i.test(o.text));
          if (opt) (sel as HTMLSelectElement).value = opt.value;
          else if ((sel as HTMLSelectElement).options.length > 1)
      ...
      await page.evaluate((note: string) => {
        const t = Array.from(document.querySelectorAll("textarea"))
          .find((x: any) => x.placeholder?.toLowerCase().includes("note"));
        if (t) {
          (t as HTMLTextAreaElement).value = note;
      ...
      await page.evaluate(() => {
        const btn = Array.from(document.querySelectorAll("button"))
          .find((b: any) => /register patient/i.test(b.textContent ?? ""));
        if (btn) btn.click();
      });
      await delay(4000);
    ...
    if (si2) {
      ...
      await page.evaluate((k: string) => {
        ...
        const m = items.find((i: any) => i.textContent?.toLowerCase().includes(k.toLowerCase()));
        if (m) (m as HTMLElement).click();
    ...
    await page.evaluate(() => {
      const btn = Array.from(document.querySelectorAll("button"))
        .find((b: any) => /book appointment|book|confirm/i.test(b.textContent ?? ""));
      if (btn) btn.click();
    });
    await delay(4000);
    ...
    console.log("Booking result:", success ? "SUCCESS" : "UNCERTAIN");
    res.json({
      success,
      message: success ? "Booking completed!" : "Submitted - please verify in AnyPlusPro",
    });
